...
@section('content')
    ...
    {!! Form::open(['method'=>'POST','action'=>'Locals\TitheController@excelMApost','class'=>'modal form-row','id'=>'date', 'tabindex'=>'-1','aria-hidden'=>'true','role'=>'dialog'] ) !!}
            ...
            <div class="panel mb25">
                <div class="panel-heading border">
                    <ol class="breadcrumb mb0 no-padding">
                        <li>{{$year}}</li>
                        <li>
                            <a  href="javascript:;" >MARCH-APRIL</a>
                        </li>
                        ...
                        <li>
                            <a class="btn btn-danger btn-xs" href="{{route('excelMA',$year)}}">Export to Excel</a>
                        </li>
                    </ol>
                </div>
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover table-responsive-sm table-sm table-small-font">
                            <thead>
                            <tr>
                                <th colspan="7" class="text-center">MARCH</th>
                                <th colspan="7" class="text-center">APRIL</th>
                            </tr>
                                    ...
                                    @foreach($users as $user_id)
                                      ...
                                      App\PostTithe::where('user_id', $user_id->id)
                                         ...
                                         ->where('local_id', Auth::user()->local_id)->pluck('amount')->sum(),2)==0.00)
                                            ...
                                            App\PostTithe::orderBy('created_at','asc')->where('user_id', $user_id->id)
                                            ->whereYear('created_at',$year)->whereMonth('created_at',$b)
                                              ->whereDay('created_at','>=',26)->whereDay('created_at','<=',30)
                                          ->where('local_id', Auth::user()->local_id)->pluck('amount')->sum(),2)==0.00)
